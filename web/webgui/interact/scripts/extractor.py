...
class Parameter(ABC):
    def _select(self, table: Table) -> Select:
        return select(
            table.c.valid_from, table.c.valid_to, table.c.version, table.c.remarks
        )
...
class DateTimes(Parameter):
    ...
    def where(self, table: Table) -> Select:
        return (
            super(DateTimes, self)
            ._select(table)
            .where(
                table.c.valid_from <= self.param[0], table.c.valid_to >= self.param[1]
            )
        )
...
class Extractor(object):
    ...
    def process(self, param: Parameter) -> pandas.DataFrame:
        if not isinstance(param, Parameter):
            raise ValueError(
                f"{param} is not an instance of the subclass of {Parameter}"
            )
        versions = {}
        for table_name, table in self.tables.items():
            s = param.where(table)
            results = connection.execute(s).fetchall()
            versions[table_name] = frozenset(VersionDetails(*res) for res in results)
