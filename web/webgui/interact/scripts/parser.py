...
class Tokenizer(object):
    ...
    groups = (("CLASS", r"\[\w+\]"), ("PARAMS", r"[^\[\]]*"))
    regex = re.compile(
        pattern="|".join(f"(?P<{group[0]}>{group[1]})" for group in groups), flags=re.M
    )
    ...
    def __iter__(self):
        for match in self.regex.finditer(self.text.strip()):
            ...
            value = match.group()
            yield Token(kind=kind.strip(), value=value.strip())
...
def write_to_database(
    ...
    table_names = set(
        x
        for x in chain.from_iterable(table_names)
        if not any(re.match(y, x) for y in EXCLUDE)
    )
    seen_tables = set()
    with open(conf_file_path) as file:
        ...
        for token in tokenizer:
            if token.kind == "CLASS":
                ...
                if current_table not in table_names:
                    raise ValueError(
                        f"there is no such table in the database: {current_table}"
                    )
        ...
        if seen_tables != table_names:
            raise ValueError(
                f'The configurations for tables {", ".join(table_names - seen_tables)} where not found'
            )
        converter.save(
            release=release,
            version=version,
...
class ValidityDates(object):
    ...
    def _(self, arg: Runs) -> Tuple[datetime, datetime]:
        ...
        if not run_from:
            raise ValueError(
                "incorrect run-from id (there are no such runs in the database)."
            )
        if len(run_from) > 1:
            raise ValueError(
                "incorrect run-from id (there are more than 1 run with such an id)."
            )
        ...
        if not run_to:
            raise ValueError(
                "incorrect run-to id (there are no such runs in the database)."
            )
        if len(run_to) > 1:
            raise ValueError(
                "incorrect run-to id (there are more than 1 run with such an id)."
            )
        run_to = run_to.first()
    ...
    def _(self, arg: Filenames) -> Tuple[datetime, datetime]:
        ...
        if not run_from:
            raise ValueError(
                "incorrect run-from filename (there are no such runs in the database)."
            )
        if len(run_from) > 1:
            raise ValueError(
                "incorrect run-from filename (there are more than 1 run with such a filename)."
            )
        run_from = run_from.first()
        valid_from = run_from.start_time
        ...
        if not run_to:
            raise ValueError(
                "incorrect run-to filename (there are no such runs in the database)."
            )
        if len(run_to) > 1:
            raise ValueError(
                "incorrect run-to filename (there are more than 1 run with such a filename)."
            )
        run_to = run_to.first()
        valid_to = run_to.start_time
