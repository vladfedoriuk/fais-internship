...
class ParseForm(forms.Form):
    ...
    def clean(self):
        ...
        if search_dates and (
            (valid_from_date and not valid_to_date)
            or (valid_to_date and not valid_from_date)
        ):
            raise ValidationError(
        ...
        if (
            search_runs
            and (not filename_from and filename_to)
            or (filename_from and not filename_to)
        ):
            raise ValidationError(
