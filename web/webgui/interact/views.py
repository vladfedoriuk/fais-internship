...
from .forms import ExtractForm, DownloadForm, ParseForm, ReleaseForm, LoginForm
...
class PaginatedMixin:
    ...
    def get_paginator(
        self, queryset: QuerySet, page: Optional[int]
    ) -> Tuple[Page, List]:
        ...
        page_range = paginator.page_range[
            page - 1 : page + self.pages_per_pagination - 1
        ]
...
def main(request):
    return render(request, "interact/base.html")
...
class FilesListView(PaginatedMixin, InteractLoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            self.template_name,
            {"page_obj": releases, "page_range": page_range},
        )
...
class ParseView(InteractLoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, {"parse_form": parse_form})
    ...
    def post(self, request, *args, **kwargs):
            ...
            if not valid_from and not valid_to:
                ...
                if arg:
                    try:
                        ...
                        error = (
                            "Unable to extract validity dates from given parameters.",
                        )
                        return render(
                            request,
                            self.template_name,
                            {"parse_form": parse_form, "error": error},
                        )
        ...
        return render(
            request, self.template_name, {"parse_form": parse_form, "sent": sent}
        )
...
class ExtractView(InteractLoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, {"extract_form": extract_form})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if extract_form.is_valid():
            ...
            if version:
                ...
                if version_exists:
                    versions_table = extractor.filter_by_version(
